...
router.get('/', async (req, res) =>{
    ...
    if(!users) return res.status(404).send("No se encontraron usuarios!");
    res.send({users})
});
...
router.post('/', async(req, res) => {
    ...
    if (user) return res.status(400).send('usuario registrado');
    user = new User({
    ...
    if(!user) return res.status(404).send("No se guardó el usuario!");
